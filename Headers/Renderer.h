...
public:
	...
	void render(GameObject* object, GameObject* camera) {
		...
		glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &camera->camera->getProjectionMatrix()[0][0]);
		...
		glm::vec4 pointA(object->glDrawable->coords[0], object->glDrawable->coords[1], object->glDrawable->coords[2], 1);
