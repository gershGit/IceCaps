...
void AddPoint(int length, float resolution, int xVal, int zVal, unsigned char * image, int width, float maxHeight, std::vector<float> &coordinates)
{
	...
	//TODO fix this
	tangent = glm::vec3(2.0f, center_r - center_l, 0.0f);
	bitangent = glm::vec3(0.0f, center_d - center_u, 2.0f);
	float x_diff = center_l - center_r;
	float z_diff = center_d - center_u;
	float y_val = 0.04f - (abs(center_l - center_r) + abs(center_d - center_u));
...
{
	int xVal = (int) (heightmap->width/2.0f + x_pos );
	int zVal = (int) (heightmap->width/2.0f + z_pos );
