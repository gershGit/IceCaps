...
void cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
	float xdif = (float) xpos - mainCamera->camera->lastXpos;
	mainCamera->camera->lastXpos = (float) xpos;
	mainCamera->rot.y -= (xdif / 800) * 2;

	float ydif = (float) ypos - mainCamera->camera->lastYpos;
	mainCamera->camera->lastYpos = (float) ypos;
...
void runUpdates() {
	/*if (GameTimer::GetTotalElapsed() - lastSpawnTime > 0.125f) {
		...
		lastSpawnTime = GameTimer::GetTotalElapsed();
	}*/
	if (GameTimer::getTotalElapsed() > 2 && !tempSwitch) {
		tempSwitch = true;
	...
	if (input->isDown(UP_ARROW_KEY)) {
		objects[1]->pos.y += 2.0f* GameTimer::getDeltaTime();
		objects[1]->moved = true;
	}
	if (input->isDown(DOWN_ARROW_KEY)) {
		objects[1]->pos.y -= 6.0f* GameTimer::getDeltaTime();
		objects[1]->moved = true;
	}
	if (input->isDown(LEFT_ARROW_KEY)) {
		objects[1]->pos.x -= 6.0f * GameTimer::getDeltaTime();
		objects[1]->moved = true;
	}
	...
	if (input->isDown(P_KEY)) {
		objects[1]->pos.z -= 6.0f * GameTimer::getDeltaTime();
		objects[1]->moved = true;
	}
	...
	if (input->isDown(W_KEY)) {
		mainCamera->pos += mainCamera->forward() * 2.0f * (float)GameTimer::getDeltaTime();
		mainCamera->moved = true;
	}
	...
	if (input->isDown(A_KEY)) {
		mainCamera->pos += mainCamera->right() * 2.0f * (float)GameTimer::getDeltaTime();
		mainCamera->moved = true;
	}
	...
	//mainCamera->pos.x = sin(GameTimer::GetTotalElapsed() * 0.5) * 50;
	//mainCamera->pos.z = cos(GameTimer::GetTotalElapsed() * 0.5) * 50;
	//mainCamera->rot.y = GameTimer::GetTotalElapsed() * 0.5f;
	//mainCamera->moved = true;
...
void renderScene() {
	...
	for (GameObject* object : renderList) {
		if (object->drawFlag) {
			if (object->name != "Ground") {
				//object->rot.y += 1.0f * GameTimer::GetDeltaTime();
				object->moved = true;
			}
	...
	for (GameObject* object : lights) {
		object->pos.y = sin(GameTimer::getCurrentTime())*3+3;
