...
void VulkanDrawable::prepare()
{
	...
	for (int i = 0; i < rendererObj->getSwapchain()->scPublicVars.colorBuffer.size(); i++) {
		...
		CommandBufferManager::allocCommandBuffer(&deviceObj->device, *rendererObj->getCommandPool(), &vecCmdDraw[i]);
		CommandBufferManager::beginCommandBuffer(vecCmdDraw[i]);

		// Create the render pass instance 
		recordCommandBuffer(i, &vecCmdDraw[i]);

		// Finish the command buffer recording
		CommandBufferManager::endCommandBuffer(vecCmdDraw[i]);
	}
}
...
void VulkanDrawable::render()
{
	...
	CommandBufferManager::submitCommandBuffer(deviceObj->queue, &vecCmdDraw[currentColorImage], &submitInfo);
